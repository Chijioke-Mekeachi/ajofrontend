...
export function useTwoFactor() {
  ...
  const { data: status, isLoading: statusLoading } = useQuery<TwoFactorStatus>({
    ...
    queryFn: async () => {
      ...
      const data = await apiPost<{ success: boolean; data: TwoFactorStatus; error?: string }>(
        "/api/totp-setup",
        { action: "status" }
      );

      if (!data.success) throw new Error(data.error || "Failed to fetch 2FA status");
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      const data = await apiPost<{ success: boolean; data: TwoFactorSetupData; error?: string }>(
        "/api/totp-setup",
        { action: "generate" }
      );

      if (!data.success) throw new Error(data.error || "Failed to generate secret");
  ...
  const enableMutation = useMutation({
    mutationFn: async (code: string) => {
      const data = await apiPost<{ success: boolean; data: { backupCodes: string[] }; error?: string }>(
        "/api/totp-setup",
        { action: "enable", code }
      );
...
export async function verifyTwoFactorCode(
  ...
  try {
    const data = await apiPost<{
      success: boolean;
      data: { verified: boolean; twoFactorRequired: boolean };
      error?: string;
    }>("/api/totp-verify", { user_id: userId, code, purpose }, { auth: false });
...
export async function check2FAStatus(userId: string): Promise<boolean> {
  try {
    const data = await apiPost<{ success: boolean; data: { isEnabled: boolean }; error?: string }>(
    ...
    if (!data.success) {
      console.error("Failed to check 2FA status:", data.error);
      return false;
    }
